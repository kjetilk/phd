...
This term is motivated by that POST operations in \cite{graphstore} is
specified to results in an RDF Merge of the payload into the named
...
In the \cite{linkeddataissue} Tim Berners-Lee
notes ``To make the data be effectively linked, someone who only has
the URI of something must be able to find their way the SPARQL
endpoint.'', but this is generally not possible today without
