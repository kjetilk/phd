...
e.g. USEWOD~\cite{eps385344} or the Linked SPARQL SPARQL Queries
...
tackle the whole load. To achieve this, a fractional factorial
experiment would likely result where certain combinations would be
banned. I did not think this complete through to a concrete
experiment. 
...
that downloading the triple pattern fragment \sparql{?s foaf:name
  ?name .}  resulted in more than 40~000 HTTP requests due to
...
``Implement'' factor in \cite{kjernsmo_doe_intro}. That is, if the
effect of this factor and its interactions are negative\todo{finish a
  short story about testing}.
