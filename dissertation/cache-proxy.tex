...
Section~\ref{sec:relcache}: a SPARQL query cache should be oblivious
...
understanding of the needs of the client, and could possibly implement
...
to the client, but will enter the cache. We then assume that the analyser
...
queries, TPF requests or a combination thereof, depending on what the
...
As can be imagined from the fact that a BGP with just two triple
...
such functionality to be present. For instance, a quadstore is
...
\begin{itemize}
\item \pmodule{AtteanX::QueryPlanner::Cache},
\item \pmodule{AtteanX::QueryPlanner::Cache::LDF} and
\item \pmodule{AtteanX::Query::Cache::Analyzer::QueryPlanner}.
\end{itemize}
Each is an
...
\item \pmodule{Attean::API::SimpleCostPlanner},
\item \pmodule{AtteanX::API::JoinRotatingPlanner},
\item \pmodule{AtteanX::Query::AccessPlan::SingleQuadBGP} and
\item \pmodule{AtteanX::Query::AccessPlan::Cache},
\end{itemize}
...
\pmodule{AtteanX::Query\linebreak[4]Planner::Cache} adjacent quad patterns are
...
coalesced by the implementation in the role \pmodule{AtteanX::API::JoinRotatingPlanner} should be
...
2 & 77.4s & 4540s & \pcode{Attean::Plan::HashJoin::\_\_ANON\_\_[...]} \\
...
4217968	        & 48.7s	& 1463s & \pcode{AtteanX::Query::AccessPlan::Cache[...]} \\
24186545	& 48.1s	& 56.6s & \pcode{Role::Tiny::does\_role} \\
28128408	& 44.4s	& 53.8s	& \pcode{Attean::Result::value} \\
16871877	& 44.1s	& 470s & \pcode{Attean::Literal::\_\_ANON\_\_[(eval 234)[...]}\\
4217969		& 40.0s & 92.0s & \pcode{Attean::API::Literal::\_\_ANON\_\_[...]} \\
...
implementation of the standard that does syntax based resolution
if an object is constructed with a base. IRI parsing happens not
...
\begin{itemize}
\item \pmodule{RDF::LinkedData},
\item \pmodule{RDF::Generator::Void},
\item \pmodule{Test::RDF},
\item \pmodule{URI::NamespaceMap} and
\item \pmodule{RDF::NS::Curated}.
\end{itemize}
