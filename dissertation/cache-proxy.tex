...
install the system on the top of a Debian system with only essential packages requires 146
Debian packages. As such, it builds on the code of hundreds of
authors, too long to enumerate. I shall constrain myself to list the
modules that have code that has been motivated from this
...
has been written my Gregory Todd Williams. The main work in this
project has gone into the module AtteanX::Query::Cache, where I have
authored the vast majority of the code. The relative difference in
authorship can be further examined by checking the linked Github
repositories, or by using git2prov\footnote{see
  \url{http://git2prov.org/}} to create RDF data of the commit
history. Table~\ref{tab:modules} shows the modules that have been
influenced or written as part of this project.
...
\begin{tabular}{ | l | p{3cm} | l |}
  ...
  AtteanX::Query::Cache & Kjetil Kjernsmo, Gregory Todd Williams &
  \url{kjetilk/p5-atteanx-query-cache} \\

  AtteanX::Store::SPARQL & Kjetil Kjernsmo &
  \url{kjetilk/p5-atteanx-store-sparql} \\
  
  AtteanX::Store::LDF & Kjetil Kjernsmo, Patrick Hochstenbach &
  \url{phochste/AtteanX-Store-LDF} \\

  LWP::UserAgent::CHICaching & Kjetil Kjernsmo &
  \url{kjetilk/p5-lwp-useragent-chicaching} \\
  
  Attean & Gregory Todd Williams, Kjetil Kjernsmo &
  \url{kasei/attean} \\

  AtteanX::Endpoint & Gregory Todd Williams &
  \url{kasei/atteanx-endpoint} \\

  RDF::LDF &  Patrick Hochstenbach, Gregory Todd Williams, Jakob Voß,
...
the project: RDF::LinkedData, RDF::Generator::Void, Test::RDF,
URI::NamespaceMap and RDF::NS::Curated.
