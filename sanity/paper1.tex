...
responses, with optional clauses matching the standard-compliant
computed freshness lifetime (which is the ideal) as well as whether
the response had other promising features that may assist caching such
as modification time, certain predicates, etc.%\todo{promising}

For each unique \httph{Server} header, we found the ones where
\emph{all} responses had a freshness lifetime or a promising feature. For
...
both the cases of standard-compliant freshness lifetime and for the other
...
should be assessed, possibly using contigency tables similar to those
