...
series of data reduction steps. Due to the presence of invalid RDF, we
...
the \texttt{sd:endpoint}\todo{footnote} or matched a case-insensitive
regular expression \texttt{sparql} if the subject and objects were
both resources, or the predicates \texttt{void:vocabulary},
\texttt{rdfa:vocabulary} or \texttt{api:vocabulary} \todo{footnotes
  for all}, as well as having an resource as object. Finally,
statements with the the classes \texttt{cogs:Endpoint},
\texttt{owl:Ontology} and \texttt{voaf:Vocabulary} \todo{footnotes for
