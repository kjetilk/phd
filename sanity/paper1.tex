...
We then developed a fairly elaborate paralell spider to get the
...
The paralell spider would then launch a process per host, but each
request to one host would be delayed by 10 seconds. For each host, the
spider would go through the list of URIs. Since the BTC2014 recorded
...
but none were. Whereever the last two headers existed, we added the
...
time based on a suggestion in RFC7234, section~4.2.2, and record that.
...
For endpoints, we examine the response message, to see if there are
any results to our query, and record that if there are.
