...
\note{
  ...
  \item We first study a full factorial, then three fractional 
  \end{itemize}
}
...
\note{
  ...
  \begin{itemize}
  \item Experiment with 8 factors, 2 levels
  \item sleep to simulate an optimization that may have detrimental
    side-effects
  \end{itemize}
...
\note{
  \begin{itemize}
  \item Significance is based on Lenth's method, where he computes a
    pseudo-standard error, since we have only one replicate. This is
    based on computing the median of the absolute effects, and then
    again the median of the smallest effects.
