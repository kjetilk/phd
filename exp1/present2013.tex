...
\note{
  \begin{itemize}
  \item Natural benchmarks, query log-derived, DBPedia SPARQL
    Benchmark
  \item If your queries are all fully known, by all means, run them
  \item In most scientific relevant cases, no such assumptions can be
    made
  \item What I will present will likely create artifical workloads
  \item Use-case driven will seem artificial for everything else
  \end{itemize}
}
...
\note{
  ...
  \begin{itemize}
  \item Implemented in R, using packages \texttt{DoE.base} and
    \texttt{FrF2}.
  \item Two experiment machines, one machine running queries
  \item No warm-up, no cache busting
  \item 256 experiments in full factorial, no replications, compare
...
\note{
  ...
  \item Yes, the Implementation is better, we knew that, joins are
    naturally strong
  ...
  \item But what about ``Implement::TripleC''?
