...
i.e. we cannot change the size of the dataset without loss of
...
estimate for the effect. However, if the effect is non-linear, the
...
well represented by the sizes of datasets. Such reasoning should be
...
prohibitivly expensive when $n$ is a large number of
factor. As the evaluation of SPARQL endpoints is an inherently complex
...
For any other purpose than a rough idea of some key influencers on the
...
explanation. However, we do not see the effects that we found
...
experiment made it feasible to run with a larger dataset, this
experiment uses the original 15~MTriples dataset of
...
parameters and so on. It is advantegous to have such broad factors in
...
We are forced to conclude that the three preceeding experiments are
...
datasets used here does not imply that the approach will not work for
large datasets; properly configured, actual optimizations will give
