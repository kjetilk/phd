...
larger databases. This should cause some alarm. 
It is a fundamental property of two-level experiments that it can only
model linear effects. If the effect is truly linear, and the variance
is small, the two levels can be quite close together and still yield a
reasonable estimate for the effect, but the effect will be greater
with greater separation. Additionally if the effect is non-linear, the
choice of levels is critical. In this case, the experimenter should be
concerned that the separation between levels for the sizes of the
data-set is too small and should further investigate. Such reasoning
should be applied to all significant effects.
