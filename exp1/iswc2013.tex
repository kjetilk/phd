...
measurement of a \emph{response variable} at certain
\emph{levels}. For example, one may measure the response time of a
SPARQL endpoint by measuring it when it contains 1 or 2 million
triples. Levels are not constrained to be continous variables, they
can be discrete, or even two different instances of a class. The
ordering is not essential, but it may contribute to how easy it is to
understand the results, as we shall see. Then, the response time is
the response variable, the factor is the number of triples and there
are two levels, 1 or 2 million triples. We have chosen in this paper
to only deal with the relatively straightforward aspects of the DoE
formalism, and therefore constrain the experiment strictly to 2
levels.
...
%TODO {Simple 8 x 2 overview table?}
...
After measurements are done, we seek to estimate \emph{effects}. To
see how the \emph{main effects} can be estimated, consider a factor
$A$. We proceed to find the averages $a_1$ and $a_2$ of all measurements at level~1 and
on level~2 respectively. Then the main effect of $A$ is $a_2 - a_1$.

Similarly, \emph{interaction effects} for 2 factors $AB$ are defined
by comparing averages for equal versus non-equal levels of $A$ and
$B$.

This can be generalized to larger combinations of factors, but in
practice, this analysis is done by fitting a linear model to the data
with all factors and their interactions. It is shown in
\cite{wu2009experiments} that the effect is twice the coefficients of
the fit.
...
be found in
figures~\ref{fig:fullnormal},~\ref{fig:frac32normal},~\ref{fig:frac64normal}~and~\ref{fig:frac64hugenormal}. The
...
``Implement'' specifies a port number. On each host,
...
\begin{figure}[h!]
  \centerline{%
    \includegraphics[width=0.9\textwidth]{frac64normal.pdf}}
  \caption{A normal plot of the Fractional Factorial Experiment with
