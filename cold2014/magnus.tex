...
To evaluate the graphs we have used a method in the \textit{FrF2} R
package called \textit{DanielPlot}. This method assumes that the
...
significant.  We have also set the \textit{alpha} to $0.05$ which
...
package in R to visualize the result. The Figure \vref{3factorGraph}
shows the results.  Here we see no significant factors. Still there
are small effects from several factors. In Table \vref{3factorEffect}
...
level \textit{-vso} which has an overall faster response time.
